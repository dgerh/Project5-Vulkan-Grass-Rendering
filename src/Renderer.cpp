...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    VkDescriptorSetLayoutBinding firstLayoutBinding = {};
    firstLayoutBinding.binding = 0;
    firstLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    firstLayoutBinding.descriptorCount = 1;
    firstLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    firstLayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding secondLayoutBinding = {};
    secondLayoutBinding.binding = 1;
    secondLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    secondLayoutBinding.descriptorCount = 1;
    secondLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    secondLayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding thirdLayoutBinding = {};
    thirdLayoutBinding.binding = 2;
    thirdLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    thirdLayoutBinding.descriptorCount = 1;
    thirdLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    thirdLayoutBinding.pImmutableSamplers = nullptr;

    std::vector<VkDescriptorSetLayoutBinding> bindings = { firstLayoutBinding, secondLayoutBinding, thirdLayoutBinding };
...
void Renderer::CreateGrassDescriptorSets() {
    ...
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool;
    allocInfo.descriptorSetCount = static_cast<uint32_t>(grassDescriptorSets.size()); // one per blade
    allocInfo.pSetLayouts = layouts;
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool;
    allocInfo.descriptorSetCount = static_cast<uint32_t>(computeDescriptorSets.size()); // one per blade
    allocInfo.pSetLayouts = layouts;
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        outputBladesBufferInfo.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
        ...
        descriptorWrites[3 * i + 1].pBufferInfo = &outputBladesBufferInfo;
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
            ...
            //vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineLayout, 0, 1, &modelDescriptorSets[j], 0, nullptr);
